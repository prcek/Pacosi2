...
class MassageTypesRow extends React.Component {
  ...
  render() {
    const { activeDrag, isDragging, connectDragSource, connectDragPreview,connectDropTarget, massageType, toolbar, classes } = this.props;
    ...
    return (
      ...
      <TableRow hover={!activeDrag} className={className}>
          ...
          <TableCell className={classes.cell}> {connectDragPreview(<div>{massageType.name}</div>)} </TableCell>

          <TableCell className={classes.cell}><StatusView status={massageType.status}/></TableCell>
          <TableCell className={classes.cell}> {massageType.length} </TableCell>
